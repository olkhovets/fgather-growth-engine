...
type SendResult = { sent: number; chosen: number; skipped: number; gradedGood: number; candidates: number; minGrade: number; incentiveCount: number; styleMix: Record<string, number>; message: string };
  ...
  const send = useCallback(async () => {
    ...
    try {
      const r = await fetch("/api/send-batch", {
        ...
        body: JSON.stringify({ count: Number(count) || 0, minGrade: Number(minGrade) || 85, providerFilter: provider }),
      });
  ...
  return (
    <details className="mb-6 rounded-xl overflow-hidden" style={{ border: "1px dashed var(--accent, #6366f1)" }} open>
      ...
      <div className="px-4 py-4 space-y-4" style={{ background: "var(--surface, #16161a)" }}>
        ...
        {result && (
          <div className="rounded-lg px-4 py-3 text-sm space-y-1" style={{ background: "var(--bg, #111)", color: "var(--text-primary)" }}>
